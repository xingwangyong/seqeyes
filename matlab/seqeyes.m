...
if ispc
    ...
    % seqeyes_exe = 'C:\path\to\seqeyes\out\bin\seqeyes.exe';
    seqeyes_exe = 'C:\Users\76494\Downloads\New folder (22)\seqeye\out\build\x64-Release\Release\seqeyes.exe';
    % seqeyes_exe = '__SET_WINDOWS_SEQEYES_PATH__';
    launch_prefix = '';
elseif ismac
    % Example macOS app-bundle path from this repo build:
    % seqeyes_exe = '/Users/yourname/Code/seqeyes/out/bin/seqeyes.app/Contents/MacOS/seqeyes';
    % folder = '~/seqeyes/';  % Parent folder of seqeyes repo
    % seqeyes_exe = fullfile(folder, 'out/bin/seqeyes.app/Contents/MacOS/seqeyes');
    seqeyes_exe = '__SET_MACOS_SEQEYES_PATH__';
    launch_prefix = '';
else
    % Example Linux path:
    % seqeyes_exe = '/home/yourname/seqeyes/out/bin/seqeyes';
    folder = '/autofs/cluster/berkin/xingwang/share/seqeyes/linux';
    seqeyes_exe = fullfile(folder, get_linux_major_id, 'seqeyes');
...
seqeyes_exe = expand_user_path(seqeyes_exe);

if ~isfile(seqeyes_exe)
...
if nargin > 0
    ...
    cmd_args = [launch_prefix, seqeyes_exe];
    cmd_args = [cmd_args, ' &'];
    fprintf('Opening SeqEyes (no input)\n');
    run_seqeyes_command(cmd_args, false);
    return;
end
...
run_in_background = should_run_in_background(normalized_options);
if run_in_background
    cmd_args = [cmd_args, ' &'];
end

%% Execute
run_seqeyes_command(cmd_args, ~run_in_background);

end


function run_seqeyes_command(cmd_args, echo_output)
%RUN_SEQEYES_COMMAND Execute SeqEyes and report shell failures clearly.

fprintf('Command: %s\n', cmd_args);
[status, cmdout] = system(cmd_args);
if echo_output && ~isempty(cmdout)
    fprintf('%s', cmdout);
...
function out = should_run_in_background(options)
%SHOULD_RUN_IN_BACKGROUND Keep CLI-style invocations attached to MATLAB.

    foreground_flags = { ...
        '-h', '--help', ...
        '-v', '--version', ...
        '--headless', '--exit-after-load', ...
        '--automation', '--capture-snapshots' ...
    };

out = true;
for i = 1:length(options)
    opt = options{i};
    if ischar(opt) && any(strcmp(opt, foreground_flags))
        out = false;
        return;
    end
end
