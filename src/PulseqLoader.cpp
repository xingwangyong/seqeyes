...
{
    if (m_sPulseqFilePath.isEmpty() || !hasLoadedSequence())
    {
        stopFileWatching();
        return;
    }
    ...
    LogManager::getInstance().appendStructured(
        QtInfoMsg,
        QStringLiteral("AutoReload"),
        QStringLiteral("Watching file for changes: %1").arg(path));
    startPendingFileHash(path, m_loadedFileSize, m_loadedLastModified);
}
...
void PulseqLoader::startPendingFileHash(const QString& path, qint64 size, const QDateTime& lastModified)
{
    ...
    if (size >= 0 && size <= kSynchronousFileHashThresholdBytes)
    {
        LogManager::getInstance().appendStructured(
            QtDebugMsg,
            QStringLiteral("AutoReload"),
            QStringLiteral("Hashing file synchronously: %1 (%2 bytes)").arg(path).arg(size));
    ...
    LogManager::getInstance().appendStructured(
        QtDebugMsg,
        QStringLiteral("AutoReload"),
        QStringLiteral("Hashing large file in background: %1 (%2 bytes)").arg(path).arg(size));
...
void PulseqLoader::handleFileHashResult(const FileHashResult& result)
{
        ...
        info.size() != result.size || info.lastModified() != result.lastModified)
    {
        LogManager::getInstance().appendStructured(
            QtDebugMsg,
            QStringLiteral("AutoReload"),
            QStringLiteral("Hash result is stale; waiting for file to stabilize again: %1").arg(path));
        if (m_fileReloadDebounceTimer)
            m_fileReloadDebounceTimer->start();
        return;
    }
    ...
    if (result.hash.isEmpty() || result.hash == m_loadedFileHash)
    {
        LogManager::getInstance().appendStructured(
            ...
            QStringLiteral("AutoReload"),
            result.hash.isEmpty()
                ? QStringLiteral("Change ignored because the file hash could not be read: %1").arg(path)
                : QStringLiteral("Change ignored because file content is unchanged: %1").arg(path));
...
void PulseqLoader::onWatchedFileChanged(const QString& path)
{
    ...
    LogManager::getInstance().appendStructured(
        QtInfoMsg,
        QStringLiteral("AutoReload"),
        QStringLiteral("File change notification received: %1").arg(currentPath));
...
void PulseqLoader::processFileChangeNotification()
{
        ...
        m_mainWindow && !m_mainWindow->isActiveWindow())
    {
        LogManager::getInstance().appendStructured(
            ...
            QStringLiteral("Changed file detected while inactive; waiting for window activation before prompting."));
        return;
    }
    ...
    if (!info.exists() || !info.isFile())
    {
        if (++m_fileReloadRetryCount <= 3 && m_fileReloadDebounceTimer)
        {
            LogManager::getInstance().appendStructured(
                QtDebugMsg,
                QStringLiteral("AutoReload"),
                QStringLiteral("Changed file is temporarily unavailable; retrying: %1").arg(path));
            m_fileReloadDebounceTimer->start();
            return;
        }
        LogManager::getInstance().appendStructured(
            QtWarningMsg,
            QStringLiteral("AutoReload"),
            QStringLiteral("Changed file is unavailable after retries; reload canceled: %1").arg(path));
        m_pendingChangedPath.clear();
        startFileWatching();
        return;
    }
    ...
    if (m_pendingFileSize != size || m_pendingLastModified != modified)
    {
        ...
        LogManager::getInstance().appendStructured(
            QtDebugMsg,
            QStringLiteral("AutoReload"),
            QStringLiteral("Waiting for file metadata to stabilize: %1 (%2 bytes)").arg(path).arg(size));
        if (m_fileReloadDebounceTimer)
            m_fileReloadDebounceTimer->start();
        return;
    }
...
void PulseqLoader::promptForChangedFileReload(const QString& path, const QByteArray& newHash)
{
    LogManager::getInstance().appendStructured(
        QtInfoMsg,
        QStringLiteral("FileMonitor"),
        QStringLiteral("Changed file content detected; asking user whether to reload: %1").arg(path));
    ...
    {
        executeFileWatcherReload(path, false);
        return;
    }
...
void PulseqLoader::executeFileWatcherReload(const QString& path, bool silent)
{
    ...
    LogManager::getInstance().appendStructured(
        QtInfoMsg,
        QStringLiteral("AutoReload"),
        QStringLiteral("Reloading changed file: %1").arg(path));
...
bool PulseqLoader::failLoad(const LoadError& error)
{
    ...
    if (m_silentMode) {
        ...
        if (m_loadUi)
            m_loadUi->showCritical(error.title, error.message);
    }
    return false;
}
...
void PulseqLoader::finishSuccessfulLoad(const QString& path, const QPair<double, double>& initialRange)
{
    ...
    if (m_loadUi)
        m_loadUi->refreshTrajectoryPlot();
    if (m_mainWindow)
    {
        // Show "SeqEyes - file.seq" only after a successful load.
        if (m_loadUi)
            m_loadUi->setLoadedTitle(path);
    ...
    if (m_isFileWatcherReload && m_mainWindow)
    {
        LogManager::getInstance().appendStructured(
            QtInfoMsg,
            QStringLiteral("AutoReload"),
            QStringLiteral("Auto reload completed: %1").arg(path));
