...
void showVersion()
{
    qDebug().noquote() << seqeyesVersionSummary();
...
int main(int argc, char *argv[])
{
    ...
    // On Windows, GUI-subsystem apps may show a message box for --help/--version; print to stdout and exit instead.
    {
        ...
        if (wantsCliHelpOrVersion(argc, argv, wantHelp, wantVersion))
        {
            ...
            coreApp.setApplicationVersion(seqeyesAppVersionPlain());
            ...
            if (wantVersion)
            {
                cliWriteText(seqeyesCliVersionText(), false);
                return 0;
            }
        }
    }
    ...
    app.setApplicationVersion(seqeyesAppVersionPlain());
    QIcon appIcon;
#ifdef Q_OS_LINUX
    // Linux: prefer the SVG resource directly for window/app icon.
    appIcon = QIcon(QStringLiteral(":/images/logo.svg"));
#else
    appIcon = QIcon(QStringLiteral(":/images/logo.svg"));
    if (appIcon.isNull())
