...
def main() -> int:
    ...
    exe = detect_exe(args.bin_dir, "PulseqOpenControllerFakeUiTest")
    env, _platform, platform_plugins = qt_test_env(exe, args.bin_dir, args.qt_bin)
    print_qt_test_header("open-controller-fake-ui", exe, env, platform_plugins)
    return run_qt_test("open-controller-fake-ui", exe, env, args.timeout)
